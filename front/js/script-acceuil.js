...
function affichageNumeroTel(json){
    let num=json.match(/.{1,2}/g);
    return num.join(' ');
};
...
function updateAccueil(){
    ...
    xhr1.onreadystatechange=function(){
        ...
        if(xhr1.readyState==4 && xhr1.status==200){
            ...
            jsonUser[3].innerText=affichageNumeroTel(json1.telephone);
            console.log(json)
        }
        else{
    ...
    xhr1.open("GET","../../back/accueilBDD.php",false);
    xhr1.send();
}
...
function affichageRdvAccueil(){
    ...
    xhr.open("GET","vosrdv.html",false);
    xhr.send();
}
...
btnPrendreRdv.addEventListener('click',()=>{
        ...
        xhr.open("GET","appointment.html",false);
        xhr.send();
...
btnConsulterMedecin.addEventListener('click',()=>{
    ...
    xhr.open("GET","consultation_medecin.html",false);
    xhr.send();
});
